...
8.1 派工前先在任务板创建任务，并把 `task_id` 绑定到 bus 消息：
   - `python3 ./scripts/tasks.py add --session {{SESSION_ID}} --title "<...>" --owner <role> --accept "<...>"`
   - `./scripts/bus-send.sh --session {{SESSION_ID}} --from lead --to <role> --intent implement --task <task_id> --message "<...>"`
