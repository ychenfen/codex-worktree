...
```bash
./scripts/autopilot.sh start fix-uds-timeout 2 --serial
```
...
```bash
./scripts/bus-send.sh --session fix-uds-timeout --from lead --to builder-a --intent implement --task <task_id> --message "实现 xxx" --accept "pytest -q" --risk medium
```
...
`/bootstrap` 行为（对标 team 模式）：
- 读取 `shared/task.md`
- 自动创建任务图（implement -> review/test）
- 自动先派发可执行任务，后续任务在依赖满足后自动派发

任务状态机（对标 Agent Teams 的任务板）：

```bash
python3 ./scripts/tasks.py list --session demo-team-20260213
python3 ./scripts/tasks.py add --session demo-team-20260213 --title "实现 README 小节" --owner builder-a --accept "README 出现标题"
```
